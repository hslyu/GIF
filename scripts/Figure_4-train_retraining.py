...
# Training
def train(net, dataloader, optimizer, criterion, label):
    net.train()
    train_loss = 0
    correct = 0
    total = 0
...
def test(net, net_name, dataloader, criterion, epoch, configs, label):
    global best_loss
    global count
    ...
    if test_loss < best_loss:
        ...
        print(
            f"checkpoints/{configs.path}/{net_name}/{configs.criterion}/ckpt_{configs.alpha}_retrained.pth"
        )
        state = {
            "net": net.state_dict(),
            "loss": test_loss,
...
def main():
    ...
    for epoch in range(start_epoch, start_epoch + configs.num_epoch):
        print("\nEpoch: %d" % epoch)
        train(net, train_loader, optimizer, criterion, 8)
        train(net, val_loader, optimizer, criterion, 8)
        early_stopping_flag = test(
            net, net_name, test_loader, criterion, epoch, configs, 8
        )
        scheduler.step()
